...
void main() {
  XmlElement parseRun(String xml) {
  ...
  group('RunParser - text extraction', () {
    test('extracts plain text', () {
      final run = parseRun('<w:r><w:t>Hello</w:t></w:r>');
    ...
    test('extracts text with spaces preserved', () {
      final run =
          parseRun('<w:r><w:t xml:space="preserve"> Hello </w:t></w:r>');
      final result = RunParser.parse(run);
      expect(result.text, ' Hello ');
    });

    test('returns empty text for empty run', () {
      final run = parseRun('<w:r></w:r>');
      final result = RunParser.parse(run);
      expect(result.text, '');
    });
  });
  ...
  group('RunParser - formatting', () {
    test('parses bold', () {
      final run = parseRun('<w:r><w:rPr><w:b/></w:rPr><w:t>Bold</w:t></w:r>');
      final result = RunParser.parse(run);
    ...
    test('parses italic', () {
      final run = parseRun('<w:r><w:rPr><w:i/></w:rPr><w:t>Italic</w:t></w:r>');
      final result = RunParser.parse(run);
  ...
  group('RunParser - line breaks', () {
    test('parses simple line break', () {
      final run = parseRun('<w:r><w:br/></w:r>');
      final result = RunParser.parse(run);
  ...
  group('RunParser - hyperlinks', () {
    ...
    test('passes bookmark reference through', () {
      final run = parseRun('<w:r><w:t>Ref</w:t></w:r>');
